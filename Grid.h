...
public:
	...
	void EmptyGrid();
	Card* GetCard(const CellPosition& position) const;
	...
	virtual void SaveAll(ofstream& savefile, objecttype gameobject);
	int CountLadders();
	int CountSnakes();
	int CountCards();

	void ExtraDice(); //added too be called in move function when card three is applied
	                  //it minus 1 from advance current player to give him an extra turn
	Player* getNextPlayer(const CellPosition& position); //added

	~Grid(); // A destructor for any needed deallcations
    Ladder * GetLadder(const CellPosition& position) const;

    Snake* GetSnake(const CellPosition& position) const;
