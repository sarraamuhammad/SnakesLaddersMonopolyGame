...
void Output::DrawCardNumber(const CellPosition& cellPos, int cardNum) const
{
	...
	pWind->DrawInteger(GetCellStartX(cellPos), GetCellStartY(cellPos), cardNum);
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_EXIT_PLAY_MODE] = "images\\new_exit.jpg";
	///TODO: Prepare images for each menu item and add it to the list
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if (fromCell.GetCellNum() < toCell.GetCellNum() && GetCellStartX(fromCell) == GetCellStartX(toCell)) {
		...
		for (int i = y2 + UI.LadderYOffset; i < y1; i += 55)
		{
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if (fromCell.GetCellNum() > toCell.GetCellNum() && GetCellStartX(fromCell) == GetCellStartX(toCell))
	{
		...
		int px1 = x12; int py1 = y1;
		int px2 = x12 - 6; int py2 = y1 - 10;
		int px3 = x12; int py3 = y1 - 20;
		int px4 = x12 + 6; int py4 = y1 - 10;


		int px[] = { px1,px2,px3,px4 };
		int py[] = { py1,py2,py3,py4 };
		int vertices = sizeof(px - 1);

		///TODO: Draw the Polygon (diamond) representing the Snake's Head
		//       Check the snakes drawn in the project document and draw it the same way
		pWind->DrawPolygon(px, py, vertices);
